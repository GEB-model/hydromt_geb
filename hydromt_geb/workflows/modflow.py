...
def get_modflow_transform_and_shape(
    ...
    gt_modlfow = (xmin, modflow_resolution, 0, ymax, 0, -modflow_resolution)
...
def create_raster_shapefile(
    transform: Affine, shape: tuple[int, int], epsg: int
) -> gpd.GeoDataFrame:
    """
    ...
    geoms = [
        {"geometry": geom, "properties": {"cell_id": int(v)}} for geom, v in shapes
    ]
...
def create_indices(
    ...
    modflow_gdf = create_raster_shapefile(
        modflow_transform, modflow_shape, modflow_epsg
    )
    ...
    intersection["area"] = intersection.apply(
        lambda x: x.hydro_geometry.intersection(x.geometry).area, axis=1
    )
...
def create_modflow_basin(hydro_mask, intersection, MODFLOW_shape) -> np.ndarray:
    ...
    ModFlow_index = np.array(
        intersection["y_modflow"] * MODFLOW_shape[1] + intersection["x_modflow"]
    )
    hydro_index = np.array(
        intersection["y_hydro"] * hydro_mask.raster.width + intersection["x_hydro"]
    )  # associated hydro cell index
